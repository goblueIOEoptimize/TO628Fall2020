...
# because the median is below the mean the data is skewed left 
...
The average and median arrival delay for all airport and airlines in Jan 2012 was  
...
names(max(delay_airline))
max(delay_airline)
names(min(delay_airline))
min(delay_airline)  
...
```{r}
summary(nyc_plane)  
arr_delay_engine=tapply(nyc_plane$arr_delay,nyc_plane$engines, mean, na.rm = T)  
barplot(arr_delay_engine)

seats_by_engine = tapply(nyc_plane$seats,nyc_plane$engines, mean, na.rm = T) 
barplot(seats_by_engine)

nyc_2013_8 = nyc_plane %>% filter( month == 8, year.x == 2013, carrier == "UA", origin == "JFK") 
